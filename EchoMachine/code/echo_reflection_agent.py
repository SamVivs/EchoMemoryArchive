...
def write_reflection_md(tag, summary):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"[EchoReflectionAgent] Reflection saved to {filename.name}")

# === AGENT LOOP === #
def main():
    print("[EchoReflectionAgent] Running and watching for tag-based reflection tasks...")
    while True:
        task = check_for_summary_task()
        if task:
            tag = task.get("tag", "").strip()
            print(f"[Task] Reflecting on tag: {tag}")
            chunks = get_content_by_tag(tag)
            text = "\n\n".join(chunks)[:6000]
            summary = summarize_with_mistral(text)

            payload = {
                "source": "echo_reflection_agent",
                "tag": tag,
                "summary": summary
            }
            ...
            clear_task()
            print("[EchoReflectionAgent] Task complete. Waiting...")
